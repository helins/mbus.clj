...
(s/fdef dlms-unit
  ...
  :args (s/cat :du ::mbus.interop/DlmsUnit)
  :ret  ::mbus/dlms-unit)
...
(s/fdef data-record$data-value-type

  :args (s/cat :dr$dvt ::mbus.interop/DataRecord$DataValueType)
  :ret ::mbus/value-type)
...
(s/fdef data-record$function-field

  :args (s/cat :dr$ff ::mbus.interop/DataRecord$FunctionField)
...
(s/fdef data-record$description

  :args (s/cat :dr$d ::mbus.interop/DataRecord$Description)
...
(s/fdef data-record
  ...
  :args (s/cat :dr ::mbus.interop/DataRecord)
  :ret ::mbus/record)
...
(defn data-record
  ...
  (void/assoc-some (let [value-type (data-record$data-value-type (.getDataValueType dr))]
                     (assoc (merge {::mbus/value-type     value-type
                                    ::mbus/function-field (data-record$function-field (.getFunctionField dr))
                                    ::mbus/storage-number (.getStorageNumber dr)
                                    ::mbus/description    (data-record$description (.getDescription dr))}
                                   (some-> (.getUnit dr)
                                           dlms-unit))
                            ::mbus/value
...
(s/fdef device-type
  ...
  :args (s/cat :dt ::mbus.interop/DeviceType)
...
(s/fdef secondary-address

  :args (s/cat :sa ::mbus.interop/SecondaryAddress)
...
(defn secondary-address
  ...
  {::mbus/device-id       (.getDeviceId       sa)
   ...
   ::mbus/device-type     (device-type (.getDeviceType sa))})




(s/fdef variable-data-structure

  :args (s/cat :vds ::mbus.interop/VariableDataStructure)
...
(defn variable-data-structure
  ...
  (void/assoc-some {::mbus/records           (map data-record
                                                  (.getDataRecords vds))
                    ::mbus/more-records?     (.moreRecordsFollow vds)
                    ::mbus/status            (.getStatus           vds)
                    ::mbus/access-number     (.getAccessNumber     vds)
                    ::mbus/secondary-address (secondary-address (.getSecondaryAddress vds))}
